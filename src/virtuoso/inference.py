...
from midi_utils.midi_utils import save_midi_notes_as_piano_midi
...
def save_model_output_as_midi(model_outputs, save_path, score, output_keys, stats, note_locations, 
    ...
    if mod_midi_path is not None:
        # Not working now since output_midi becomes list of tracks.
    ...
    for n_channel in range(len(output_midi)):
        output_midi[n_channel].sort(key=lambda x:x.start)
    if attention_weights is not None:
    ...
    save_midi_notes_as_piano_midi(output_midi, midi_pedals, save_path,
                                  bool_pedal=bool_pedal, disklavier=disklavier, tempo_clock=clock_notes, multi_channel=True)
